...
| Feature | Naive model (mean abs SHAP) | Corrected model (mean abs SHAP) | Change |
|---|---|---|---|
| Duration | 0.2003 | 0.2003 | Unchanged, robust signal |
| Occurrence count | 0.0744 | 0.0328 | Down ~56% |
| Lazarus flag | 0.0480 | 0.0437 | Roughly stable |
| Latitudinal range | 0.0481 | 0.0322 | Down ~33% |
| Longitudinal range | 0.0513 | 0.0283 | Down ~45% |
| Environmental breadth | 0.0236 | 0.0098 | Down ~58% |
...
│   ├── raw/                  (PBDB downloads, excluded from version control)
...
Survival labels: A genus is labeled as surviving if it appears in the subsequent time interval. Lazarus taxa (genera that disappear for one interval and reappear later) are flagged explicitly via a lazarus indicator rather than silently mislabeled as extinct. This flag is computed at the genus level (independent of interval), so it cannot leak information from the interval-specific survival label.
...
Models: Two Random Forest classifiers are trained on identical train/test splits: a naive model (biological features only) and a corrected model (biological plus sampling-control features). SHAP values are computed for both, and feature importance is compared directly to identify which biological signals are robust to sampling-bias correction.

Limitations: Coordinates used in the occurrence map (results/figures/paleogeographic_map.html) are modern present-day latitude/longitude, not plate-tectonic-reconstructed paleocoordinates, since true paleogeographic positions were not available in this data pull. The sampling-effort proxy used in the diversity comparison is occurrence count rather than strict collection count, though both serve the same diagnostic purpose.
...
Raw PBDB downloads are not included in this repository since they are large and freely re-downloadable. Run notebooks/01_data_acquisition_cleaning.ipynb first to regenerate data/raw/ and data/processed/occurrences_clean.csv from the PBDB API directly.

## How to run

Run the notebooks in order, since each depends on outputs saved by the previous one:
...
For each notebook, use Restart and Run All to ensure a clean, reproducible execution from a fresh kernel.

### Viewing results without re-running anything

Static figures are already saved in results/figures/: diversity_raw_vs_corrected.png, shap_naive_vs_corrected.png, and paleogeographic_map.html (open directly in a browser, since GitHub does not render interactive HTML inline).
...
Paleobiology Database (PBDB), https://paleobiodb.org. Occurrence and collection-level data accessed via the public Data Service API.
